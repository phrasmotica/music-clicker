...
func _on_ui_manager_buy_product(product:Product, cost:int) -> void:
	if _score < cost:
		return
...
func _on_ui_manager_made_product(reward:int) -> void:
	_score += reward
	score_changed.emit(_score)

func _on_ui_manager_automate_product(product:Product, automate_cost:int) -> void:
